...
nose_left <- synDownloadTableColumns(actv_tremor_syntable, "deviceMotion_tremor_handToNose_left.json.items")
nose_left <- data.frame(deviceMotion_tremor_handToNose_left.json.items=names(nose_left), nose_left_file=as.character(nose_left))
actv_tremor <- merge(actv_tremor, nose_left, by="deviceMotion_tremor_handToNose_left.json.items", all=T)
...
OUTPUT_FILE <- paste(currentDirectory, "/featureExtraction/TremorNoseLeftFeatures.tsv", sep="")
