...
shoulder_right <- synDownloadTableColumns(actv_tremor_syntable, "deviceMotion_tremor_handAtShoulderLength_right.json.items")
shoulder_right <- data.frame(deviceMotion_tremor_handAtShoulderLength_right.json.items=names(shoulder_right),
				shoulder_right_file=as.character(shoulder_right))
actv_tremor <- merge(actv_tremor, shoulder_right, by="deviceMotion_tremor_handAtShoulderLength_right.json.items", all=T)
...
shoulderRightFeatures <- ddply(.data=actv_tremor, .variables=colnames(actv_tremor), .parallel=runParallel,
                         .fun = function(row) { tryCatch({mpowertools::getWalkFeatures(as.character(row$shoulder_right_file))},
                                                 error = function(err){
                                                 print(paste0('Unable to process ', row$shoulder_right_file))
                                                 print(err) })
                                                })


#only keep the non-redundant data
shoulderRightFeatures <- shoulderRightFeatures %>% filter(recordId %in% selected_records)
...
OUTPUT_FILE <- paste(currentDirectory, "/featureExtraction/TremorShoulderRightFeatures.tsv", sep="")
write.table(shoulderRightFeatures, OUTPUT_FILE, sep="\t", row.names=F, quote=F, na="")
